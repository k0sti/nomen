  ...
  >
    <div class="flex items-start justify-between gap-3">
      <div class="min-w-0 flex-1">
        ...
        <p class="text-sm text-gray-400 mt-1 {isExpanded ? '' : 'line-clamp-2'}">{memory.content?.split('\n')[0] || ''}</p>
      ...
      <div class="text-right shrink-0">
        ...
        {#if memory.importance}
          <div class="text-xs text-gray-600 mt-1">importance: {memory.importance}</div>
        {/if}
      </div>
    </div>
  </button>
  ...
  {#if isExpanded}
    <div class="px-4 pb-4 border-t border-gray-800/50 pt-3 space-y-3">
      <div>
        ...
        <p class="text-sm text-gray-300 mt-1 whitespace-pre-wrap">{memory.content}</p>
      </div>
