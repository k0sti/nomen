<script lang="ts">
  ...
  async function decryptPrivateMemories(ms: Memory[], r: any): Promise<Memory[]> {
    ...
    for (const m of ms) {
      ...
      if (looksEncrypted(content)) {
        const dec = await tryDecryptWithNsec(content, m.source, nsec);
        if (dec) content = dec;
      }
  ...
  async function createMemory() {
    ...
    try {
      const signer = getSigner();
      await $relay.storeMemory(newTopic.trim(), newContent.trim(), newVisibility, signer);
  ...
  {#if showCreateForm}
    <div class="p-4 rounded-lg border border-gray-700 bg-gray-900/50 space-y-3">
      ...
      <label class="block">
        ...
        <textarea bind:value={newContent} rows="4" placeholder="Memory content (plain text/markdown)..." class="mt-1 w-full px-3 py-2 bg-gray-900 border border-gray-700 rounded-lg text-sm text-gray-200 resize-y focus:border-accent-500"></textarea>
      </label>
