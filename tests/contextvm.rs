...
async fn test_handler() -> Result<(CvmHandler, tempfile::TempDir)> {
    ...
    let handler = CvmHandler::new(nomen, vec![], 30, "nostr".to_string());
    Ok((handler, tmp))
}
...
mod policy_tests {
    ...
    async fn rate_limiter_allows_within_limit() {
        let (db, _tmp) = init_test_db().await.unwrap();
        let nomen = nomen::Nomen::from_db(db);
        let handler = CvmHandler::new(nomen, vec![], 5, "nostr".to_string());
    ...
    async fn rate_limiter_separate_per_client() {
        let (db, _tmp) = init_test_db().await.unwrap();
        let nomen = nomen::Nomen::from_db(db);
        let handler = CvmHandler::new(nomen, vec![], 2, "nostr".to_string());
