...
pub async fn prepare(
    ...
    crate::db::cleanup_expired_consolidation_sessions(db).await.ok();
...
pub async fn commit(
    ...
    if session.expires_at < now_str {
        crate::db::update_consolidation_session_status(db, session_id, "expired").await.ok();
        ...
        for memory in &extraction.memories {
            ...
            if let Ok(record_id) = get_memory_record_id(db, &stored_dtag).await {
                for msg_id in &msg_ids {
                    crate::db::create_consolidated_edge(db, &record_id, msg_id).await.ok();
                }
            }

            // Entity extraction (heuristic)
            {
                let entity_text = format!("{} {}", memory.summary, memory.detail);
                if let Ok((entities, relationships)) = config.entity_extractor.extract(&entity_text, &[]).await {
                    if let Ok(record_id) = get_memory_record_id(db, &stored_dtag).await {
                        for entity in &entities {
                            if let Ok(entity_id) = crate::db::store_entity(db, &entity.name, &entity.kind).await {
                        ...
                        for rel in &relationships {
                            crate::db::create_typed_edge(db, &rel.from, &rel.to, &rel.relation, None).await.ok();
                        }
                    }
                }
            }
    ...
    if !all_msg_ids.is_empty() {
        crate::db::mark_messages_consolidated(db, &all_msg_ids).await.ok();
    }
