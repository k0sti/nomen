...
pub async fn get_meta(db: &Surreal<Db>, key: &str) -> Result<Option<String>> {
    ...
    let result: Option<MetaRow> = db
        .query("SELECT value FROM kv_meta WHERE key = $key LIMIT 1")
...
pub async fn set_meta(db: &Surreal<Db>, key: &str, value: &str) -> Result<()> {
    let now = chrono::Utc::now().to_rfc3339();
    db.query("DELETE FROM kv_meta WHERE key = $key; CREATE kv_meta CONTENT { key: $key, value: $value, updated_at: $now }")
        .bind(("key", key.to_string()))
