...
**Tier assignment:** Currently defaults to `public`. Should be derived from the source messages' context:
- DM messages → `private`
...
This preserves provenance — you can always trace a named memory back to the raw conversation that produced it.
...
3. **Batch deletions:** Max 50 event IDs per deletion event to avoid oversized events
...
**Future:** LLM-arbitrated conflict resolution where the model assesses which version is more likely correct given full context.

## 6. Decay & Pruning

Memories that are never accessed should eventually be pruned. This prevents unbounded growth and keeps retrieval quality high.
...
This affects retrieval ranking, not storage. Low-effective-confidence memories rank lower in search results.
...
Where:
- `semantic_similarity`: cosine distance from query embedding to memory embedding
- `text_match`: BM25 score from full-text search
- `recency`: exponential decay based on `last_accessed` or `created_at`
- `importance`: `confidence` field (LLM-assigned at creation/consolidation)

Named memories naturally score higher than unconsolidated raw messages because they have higher confidence and more focused content.
...
base_url = "https://openrouter.ai/api/v1"  # optional, inferred from provider

[memory.pruning]
enabled = true
max_age_days = 90               # prune unaccessed memories older than this
min_confidence = 0.3            # prune below this confidence after 30 days
```
...
nomen list --ephemeral    # only unconsolidated raw messages
```
...
- [ ] **Tier derivation from source context** — Currently defaults to `public`. Should be `private` for DM sources, `group` for group sources.
- [ ] **Merge into existing memories** — When topic d-tag already exists, merge instead of creating duplicate. Requires fetching existing + re-prompting LLM.
- [ ] **Conflict detection** — Flag contradictions between new and existing memories. Create `contradicts` graph edges.
- [ ] **Access tracking** — `last_accessed` and `access_count` fields on memory records. Updated on search hits.
- [ ] **Confidence decay** — Time-based decay factor in retrieval scoring.
- [ ] **Pruning command** — `nomen prune` to delete unaccessed/low-confidence memories.
- [ ] **Importance scoring at creation** — LLM assigns importance (1-10) during extraction, stored alongside confidence.
- [ ] **Deduplication pass** — Embedding similarity check before creating new memories to catch near-duplicates.
- [ ] **Entity extraction during consolidation** — Extract entities (people, projects) from consolidated content, create `mentions` edges.
- [ ] **Auto-trigger** — Cron/heartbeat-based automatic consolidation when `enabled = true` and interval elapsed.
- [ ] **Cross-group consolidation guard** — Prevent private ephemeral memories from leaking into group-tier named memories.
...
The dream cycle would run after consolidation:
1. Sample random memories from different categories
2. Present unrelated memories to LLM with creative prompt
3. Store discovered associations as low-confidence `dream/*` memories
4. Promote to named memories if confirmed by later interactions
